...
class BACKUPROCKS extends BACKUPROCKS_Base {
	...
	public function backup_rocks_callback() {
		...
		<div class="wrap">
			<div class="top_navigation">
				<a href="https://backup.rocks/#plans"><img src="<?php echo plugin_dir_url(__FILE__); ?>images/backup-rocks-300x761.png"></a>				
			</div>
			<div class="backup-wrap">
				<h1>The BackupRocks plugin <strong><a>requires a subscription</a></strong></h1>
				<div class="desc">Get realtime backups,automated security scanning and support from wordpress experts.</div>
				<a target="blank" href="#" class="button-rocks">View plans and pricing</a>
			</div>

			<div class="backup-wrap2">
				<h1>Already have a BackupRocks account?</h1>
				<form method="post" action="options.php">
					<?php settings_fields('backup_rocks_license'); ?>
					<div class="desc">Paste your registration key below.</div>
	...
	function backup_rocks_license_activate() {	
		if( isset( $_POST['backup_rocks_license_activate'] ) ) {
			...
			START: $api_params;
			$response = wp_remote_post( BACKUP_ROCKS_URL, array( 'timeout' => 10, 'sslverify' => true, 'body' => $api_params ) );
	...
	function backup_rocks_rocking($option = '' ) {
		...
		$rs = wp_remote_post( BACKUP_ROCKS_URL, array( 'timeout' => 10, 'sslverify' => true, 'body' => $api_params ) );
		return false;
	}
	...
	function res_br_api_last() {
		$filtered_post = $this->filter_data( $_POST, array( 'action', 'brbr', 'url', 'key', 'form_data', 'prefix', 'type', 'location', 'tables', 'xiferp' ) );
	...
	function res_to_br_api() {
		...
		$return = apply_filters( 'wpbr_establish_remote_connection_data', $return );
